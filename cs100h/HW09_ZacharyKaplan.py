...
#The above line is required to run python on linux with unicode characters
...
''' PROBLEM 0. Put your name and the date in this
comment.
...
def litCricFriend(wordList, text):
    ...
    # One line of code.
    text = text.replace('—', ' ').replace('-', ' ') #replaces both the unicode long hyphen and the ascii dash
    ...
    # Three lines of code.
    words = [word.strip(string.punctuation) for word in words]  #reconstructs words by stripping punctuation from each element and storing back to words
    ...
    count = sum(words.count(word.lower()) for word in wordList) #generates the counts of occurrances of each word in wordList and sums all of the counts
...
print() #extra newline
print("Indefinite Articles (A and An) Frequencies:",
        "Poe\t%f" % litCricFriend(['A', 'An'], theBells),
        "Neruda\t%f" % litCricFriend(['A', 'An'], cantoXII), sep='\n\t');

print() #extra newline
print("Definite Article (The) Frequencies:",
        "Poe\t%f" % litCricFriend(['The'], theBells),
        "Neruda\t%f" % litCricFriend(['The'], cantoXII), sep='\n\t');
...
Indefinite Articles (A and An) Frequencies:
    Poe         0.034653
    Neruda      0.009615

Definite Article (The) Frequencies:
    Poe         0.135314
    Neruda      0.054487

This output shows that Poe ultimately uses articles more than Neruda. In addition to this, each author
uses definite articles far more than indefinite articles. Neruda uses 'The' over five times as much
as 'A' or 'An', whereas Poe uses 'The' about four times as much as 'A' or 'An'.
'''
