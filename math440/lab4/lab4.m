...
% despite satisfying the modulus equations above
...
% \end{aligned} $$
%
% Notice that $A \in \mathbb{R}^{2M + 4 \times (M+2)(N+2)}$, and is
% essentially two identity matrix blocks, and the 4 corner constraints.
% Finally, we can represent all constraints in the matrix system given by
%
% $$
...
%
% First we notice that if we take $T(x, y) = T^*(x, y) + 300$, then 
% we can substitute this into our PDE system to find
%
% $$ \begin{aligned}
%  \nabla^2 T = \nabla^2 T^* = 0, && \forall~(x,y) \in \Omega \\
...
% \end{aligned} $$
%
% For this new PDE system on $T^*$, we assume the form
% $T^*(x, y) = \phi(x)\cdot\psi(y)$ which gives
%
% $$ \begin{aligned}
...
% \end{aligned} $$
%
% The above is now an ODE system where $\psi$ and $\phi$ are almost
% completely decoupled. Notice that the top equation in the previous
% display implies that $\phi''/\phi = -\psi''/\psi$.
% If we take $\lambda = \phi''/\phi$, then we arrive at two ODEs:
%
% $$ \left\{ \begin{aligned}
...
% \end{aligned} $$
%
% Since we have solutions for $\psi_k~\forall~k = 0, 1, \ldots$,
% we consider solving for $\phi_k~\forall~k = 0, 1, \ldots$.
% If $k \ge 1$ then
%
% $$ \begin{aligned}
...
% \end{aligned} $$
%
% If we the consider $T^*$ to be the linear combination of all such 
% $\phi_k \cdot \psi_k$ we find that
%
% $$ \begin{aligned}
...
%%% Why is the numerical result exact?
%
% The text asks us to consider why the numerical result is exact.
% Since our analytic solution $T = 75x + 300$ is $O(1)$ w.r.t. $y$ and
% $O(x)$ w.r.t. $x$, it seems natural that a second-order FD scheme would
% converge exactly. This is effectively the equivalent of solving a
% 1D system with a linear solution using a second-order method.
%

%%% Comparison to results from LU factorization solver.
%
% Now we just repeat our numerical results with the lu solver,
% and print an extra table at the end denoting the timing differences of
% each case.
%
...
%% Helper Functions
%
% This section contains the implementation of any helper functions used
% above.
%
