...
% https://www.overleaf.com/read/qmtzkncjftck
% 
...
%% Numerical Scheme
%
% In this lab, we will consider the following explicit RK-3 scheme on a
% system of the form $u_t = f(t, u)$:
%
% $$ \begin{aligned}
%   u_k &= 
...
%% Van der Pol's DE
%
% For the first section of this lab, we will consider the following
...
% $$
%
% Using the substitution $x = y_t$, we can rewrite the above as the
% following coupled system of ODEs:
%
% $$ \left\{ \begin{aligned}
%   x_t &= (1-y^2) x - y \\
%   y_t &= x
% \end{aligned} \right. $$
%
% Therefore, if we define the vector
% $\bm{X} = \begin{pmatrix} x \\ y \end{pmatrix}$, we can describe Van der
% Pol's equation as $\bm{X}_t = f(t, \bm{X})$ where
%
% $$
%   f\left(t, \bm{X}\right) = 
...
%% C.2.1.1 Accuracy of our RK-3 Method
% 
% In order to numerically estimate the order of our method on the Van der
% Pol model, we will consider partitions of sizes 
% $N = 10, 20, 40, 80, 160, 320$ over the support $t \in [0, 1]$.
% If we write $y^{(N)}$ to be the numerical solution to Van der Pol's
% equation with partition size $N$, then we can calculate the error at 
% $t = 1$ for that partition size as $e_N = y^{(N)}_N - y(1)$.
% Since the true value of $y(1)$ is unknown, consider $y^{(320)}_{320}$ to
% be a good enough approx to $y(1)$ and use that to find the errors $e_N$
...
%% Robertson's Problem
%
% For the next section, we will consider the following coupled nonlinear
% ODE's which form a stiff problem:
%
% $$ \left\{ \begin{aligned}
...
% \end{aligned} \right. $$
%
% where $k_1 = 0.04, k_2 = 10^4, k_3 = 3 \cdot 10^7$. The above is
% trivially vectorized by considering
% $\bm{X} = \begin{pmatrix} x_1 & x_2 & x_3 \end{pmatrix}$.
%

%% C.2.1.2 Stability Investigation of our RK-3 Method
%
% Below we will examine the stability of our RK-3 method as well as the
% stability of some of matlab's built-in solvers.
%

%%% C.2.1.2.1 RK-3 w/ fixed grid size.
%
% Below we consider Robertson's problem for $t \in [0, 1]$, and grid sizes
% of $N = 125, 250, 500, 1000, 2000$. We will then (qualitiatively)
% classify the behavior for different $N$ values to determine at which
% minimal $N$ value the solution becomes stable.
...
%%%
%
% By inspection of the output of the above, we notice that the solution
% exceeds the bounds of IEEE double precision floats until $N \ge 1000$.
% Since this problem is physical, we expect that the solution should NOT
% grow greater than $\approx2^{300}$ and therefore notice that the solution
% behaves stable (bounded) for $N \ge 1000$.
% 
...
%%% C.2.1.2.2 Stability w/ Adaptive Time-Stepping
%
% First we will look at results for the non-stiff solver ode23 on $[0, 1]$,
% using $RelTol = 10^{-3}, 10^{-4}, 10^{-5}, 10^{-6}$. We'll then plot h(t)
% for the final tolerance.
%
...
%%%
%
% Next we will run the stiff ODE solver ode23s for $t \in [0, 1000]$,
...
%% C.2.1.3 Parameter Study of an ODE System
%
% For the following ODEs, we will use the classical explicit RK-4 ODE
% solver to find numerical solutions, and then visualize qualitative
% changes in the numerical solutions as certain parameters are varied.
%

%%% C.2.1.3.1 Particle Flow Past a Cylinder
%
% The position of a particle is determined uniquely by it's initial point
% $(x(0), y(0))$ and
%
% $$ \left\{ \begin{aligned}
...
%
% Consider 4 particles starting at $x = -4$ and $y = 0.2, 0.6, 1.0, 1.6$.
% We then plot the flow of each particle in $\mathbb{R}^2$ for
% $t \in [0, 10]$.
%
...
%%% C.2.1.3.2 Motion of a Particle
%
% Consider a particle thrown from (0, 1.5) at angle $\alpha$ and velocity
% $\nu_0 = 20$. The trajectory then depends on $\alpha$, the air resistance
% coefficient $k$, and the ODE system
%
% $$ \left\{ \begin{aligned}
...
% where $g \approx 9.81$.
%
% We seek to solve this system with $\alpha = 30^\circ, 45^\circ, 60^\circ$
% and $k = 0.020, 0.065$ and visualize the differences.
%
...
%
% In a matlab script file, all functions must be defined below the body of
% the script. As a result, a lot of the code that would be useful to read
% before reading the above is instead down here...
%
% Also, any figures that depend on the result of calling a function down
% here is also inherently delayed until this section has completed, often
% clustering output figures at the bottom of the file instead of inline
% with their corresponding section. :(
%

%%% RK-3 Solver
%
% This implements the explicit RK-3 scheme described in section
% ``Numerical Scheme.''
% This solver assumes an IVP on a first order ODE of the form:
% $\bm{X}_t = f(t, \bm{X}), \quad \bm{X}(0) = \bm{X}_0$.
%
...
%%% RK-4 Solver
%
% This implements the classic explicit RK-4 solver. Used in section C.2.1.3
% where I am asked to choose a solver of at least order 2 to use.
%
